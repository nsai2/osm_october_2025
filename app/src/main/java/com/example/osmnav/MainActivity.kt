...
class MainActivity : ComponentActivity(), TextToSpeech.OnInitListener {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    // Kick off routing with GraphHopper
  ...
  private val cb = object : LocationCallback() {
    override fun onLocationResult(res: LocationResult) {
      ...
      // Snap-to-route (visual only): project to nearest point on current route if close
      val (distToRoute, snap) = nearestPointOnPolyline(p, routePoints)
      val displayPoint = if (distToRoute < 20.0) snap else p  // 20m snap window

      // Auto-advance: if within 25m of next step end
      ...
      // Off-route detection: if farther than 40m from any route point -> reroute
  ...
  private suspend fun routeWithGraphHopper(start: GeoPoint, end: GeoPoint) {
    try {
      ...
      val url = "https://graphhopper.com/api/1/route?profile=car&locale=${Locale.getDefault().language}&points_encoded=false&ch.disable=true" +
                "&point=${start.latitude},${start.longitude}&point=${end.latitude},${end.longitude}&instructions=true&calc_points=true&points_encoded=false&key=${URLEncoder.encode(key, "UTF-8")}"
      ...
      // Instructions
      val instrs = path.getJSONObject("instructions").getJSONArray("instructions")

      val parsed = mutableListOf<Step>()
      for (i in 0 until instrs.length()) {
        val ins = instrs.getJSONObject(i)
        val text = ins.getString("text")
        val dist = ins.optDouble("distance", 0.0)
        val timeMs = ins.optDouble("time", 0.0)
        // endpoint: last coordinate of this interval
        val interval = ins.getJSONArray("interval")
        val endIdx = interval.getInt(1).coerceAtMost(pts.size - 1)
  ...
  private fun graphHopperKey(): String {
    // Try env var first, then local.properties (BuildConfig field not set here; adjust as needed)
  ...
  // --- Geocoding (Nominatim) ---
  private suspend fun geocodeNominatim(query: String): GeoPoint? {
    return try {
      val q = URLEncoder.encode(query, "UTF-8")
      val url = "https://nominatim.openstreetmap.org/search?q=${q}&format=json&limit=1&addressdetails=0"
      val json = URL(url).readText()
      val arr = JSONObject("{\"r\":${json}}").getJSONArray("r")
  ...
  private fun updateTtb(totalDist: Double, totalDurSec: Double) {
    ...
    ttbSecondary.text = "ETA ~ ${mins} min  •  Total ${"%.1f".format(km)} km"
  }
  ...
  private fun nearestPointOnSegment(p: GeoPoint, a: GeoPoint, b: GeoPoint): GeoPoint {
    // Approximate small segments in meters using equirectangular projection around p
